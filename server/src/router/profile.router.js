...
router.get('/search', async (req, res)=>{
    try {
        ...
        if(PROFILES.length === 0){
            let response =  await ProfileDB.find();     
            response.map(profile=>{
                PROFILES.push(profile)
            })
        }
        let result = PROFILES.filter(profile =>{
            return profile.name.toLowerCase().includes(searchKey.toLowerCase()) || profile.email.toLowerCase().includes(searchKey.toLowerCase())
        })
